...
class SearchView(LoginRequiredMixin, View):
    ...
    def get(self, request, list_number=0):
        ...
        if not message:
            dictionary['peoples'] = Person.objects.all(
            )[list_number*10:list_number*10+10]
            ...
            if search_name.isdigit():
                result = \
                    Person.objects.filter(
                        first_phone_number__contains=search_name).order_by('surname') |\
                    Person.objects.filter(
                        second_phone_number__contains=search_name).order_by('surname')
            else:
                result = \
                    Person.objects.filter(
                        surname__icontains=search_name).order_by('surname') | \
                    Person.objects.filter(
                        name__icontains=search_name).order_by('surname')
